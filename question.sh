...
#
sed 's/\/\([0-9][0-9]\);/\/20\1;/' taller.csv | sed 's/\/\([0-9]\)\//\/0\1\//' | sed 's/\(^[0-9]\)\/\([0-9][0-9]\)\/\([0-9][0-9][0-9][0-9]\)/0\1\/\2\/\3/' | sed 's/\([0-9][0-9]\)\/\([0-9][0-9]\)\/\([0-9][0-9][0-9][0-9]\)/\3\-\2\-\1/' | sed 's/,/./' | sed 's/a/A/' a5 | sed 's/n/N/' | sed 's/c/C/' | sed 's/;N/;\\N/' | sed 's/;;/;\\N;/' | sed 's/;$/;\\N/' | sed 's/;/,/g' a8 > output.csv